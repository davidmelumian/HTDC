...
class Triangle implements  IShape{
    CartPt loc;
    int size; // An isoscele right  triangle always have two sides, that are equal.The third one is called Hypotenuse.
    Triangle (CartPt loc,int size){
        this.loc = loc;
        this.size=size;
    }
    ...
    double base (){
    return  roundOff(Math.sqrt(2)*this.size);
    }
    //determines the area
    double area(){
    ...
    double perimeter (){
        return  base()+this.size+this.size;
    }
